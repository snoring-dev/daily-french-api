...
@Injectable()
export class TikTokExtractionService {
  private readonly logger = new Logger(TikTokExtractionService.name);

  constructor() {}

  async extractTikTokVideo(url: string): Promise<any> {
    try {
      return await this.extractWithYtDlp(url);
    } catch (error) {
      this.logger.warn(`TikTok yt-dlp extraction failed: ${error.message}`);
      throw new Error(`TikTok video extraction failed: ${error.message}`);
    }
  }
  ...
  private async extractWithYtDlp(url: string): Promise<any> {
    try {
      ...
      // TikTok-specific settings to bypass restrictions
      const ttArgs =
      ...
      return {
        ...
        title: fullData.title || 'TikTok Video',
  ...
  private extractVideoId(url: string): string {
    ...
    // Example URLs:
    // https://www.tiktok.com/@barrytv.official/video/7495443050701901078
    // https://vm.tiktok.com/ZGd88k8VE/

    // Try to get ID from /video/ format
    ...
    // Try to get ID from vm.tiktok.com shortened URL format
    if (url.includes('vm.tiktok.com/')) {
      // For shortened URLs, we'll use a hash of the URL as the ID
      // since we can't directly extract the numeric ID
