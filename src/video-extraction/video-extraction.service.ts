...
export class VideoExtractionService {
  ...
  getHello(): string {
    return 'Hello from Video Extraction Module!';
  }
  ...
  async extractVideo(url: string): Promise<any> {
    ...
    try {
      ...
      this.logger.warn(`yt-dlp extraction failed: ${error.message}`);
      ...
      throw new Error(`Video extraction failed after all attempts`);
    }
  }
